...
const Dashboard: React.FC<DashboardProps> = ({ profile, logs, onAddLog }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 lg:space-y-12 animate-in fade-in duration-500 relative pb-20">
      {/* Apple-style Satin Header Banner */}
      <div 
      ...
      >
            ...
            <div className="space-y-3">
              ...
              <div className="pt-4 flex flex-wrap gap-3 justify-center lg:justify-start">
                 <div className="flex items-center gap-2.5 bg-white/15 px-5 py-2.5 rounded-full backdrop-blur-md border border-white/10 shadow-sm">
                   ...
                   <span className="font-bold text-xs lg:text-sm text-white">{profile.streakCount} {t.dashboard.streakSuffix}</span>
                 </div>
                 <div className="flex items-center gap-2.5 bg-white/15 px-5 py-2.5 rounded-full backdrop-blur-md border border-white/10 shadow-sm">
                   <Star size={16} className="text-white opacity-80" fill="currentColor" />
                   <span className="font-bold text-xs lg:text-sm text-white">{profile.completedActivities.length * 20} {t.dashboard.progressSuffix}</span>
                 </div>
              </div>
            </div>
          </div>
        ...
        <div className="absolute bottom-[-10%] right-[-5%] opacity-[0.08] pointer-events-none scale-150 text-white">
          <Leaf size={300} />
        </div>
      </div>

      {/* Modern Frosted Activity Card */}
      {nextActivity && !profile.journeySettings.isPaused && (
        <div 
          className="bg-white/70 backdrop-blur-xl p-8 lg:p-10 rounded-[2.5rem] border border-white/60 shadow-[0_10px_40px_rgba(0,0,0,0.02)] flex flex-col md:flex-row items-center justify-between gap-8 group hover:shadow-[0_30px_60px_rgba(0,0,0,0.05)] transition-all duration-500 cursor-pointer"
        >
           <div className="flex items-center gap-8 w-full lg:w-auto">
              <div 
                className="h-16 w-16 lg:h-20 lg:w-20 rounded-[1.75rem] flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform shrink-0"
          ...
          <div className="h-56 lg:h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Line type="monotone" dataKey="pain" stroke="#94A3B8" strokeWidth={2} strokeDasharray="6 6" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>

        <div className="bg-white/70 backdrop-blur-xl p-8 lg:p-12 rounded-[2.5rem] shadow-[0_10px_50px_rgba(0,0,0,0.02)] border border-white/60 flex flex-col justify-between group">
           <div className="space-y-4">
             <h3 className="text-xl lg:text-2xl font-bold text-slate-900 flex items-center gap-4">
               <div className="p-3 bg-emerald-50 text-emerald-500 rounded-2xl group-hover:rotate-12 transition-transform shadow-inner"><Leaf size={24} /></div>
              ...
              {NUTRITION_GUIDE.slice(0, 2).map((item, idx) => (
                <div key={idx} className="p-6 bg-slate-50/50 rounded-[1.75rem] border border-white/80 space-y-4 hover:border-emerald-100 transition-colors">
                   ...
                   <div className="flex flex-wrap gap-2">
                     {item.items.map(i => <span key={i} className="text-[10px] bg-white px-3 py-1 rounded-full font-bold text-slate-600 shadow-sm border border-slate-100/50">{i}</span>)}
                   </div>
                   <p className="text-xs text-emerald-600 font-bold italic opacity-90 leading-tight">"{item.benefit}"</p>
                </div>
              ))}
           </div>
           <button className="w-full py-5 border-2 border-dashed border-slate-200 rounded-3xl font-bold text-sm text-slate-500 hover:border-emerald-200 hover:text-emerald-600 transition-all active:scale-95">
              {t.dashboard.recipes}
           </button>
        </div>
      </div>
...
const StatCard = ({ icon, label, value, unit = "", color }: any) => (
  <div className={`p-8 lg:p-10 rounded-[2.5rem] shadow-sm border border-white/60 flex flex-col justify-between hover:translate-y-[-6px] transition-all duration-500 cursor-default group ${color} backdrop-blur-sm`}>
    <div className="flex items-center gap-4 text-slate-500 font-bold mb-8">
      <div className="p-3 bg-white/80 rounded-2xl shadow-sm group-hover:scale-110 transition-transform">{icon}</div>
      <span className="text-[10px] uppercase tracking-[0.2em] line-clamp-1 opacity-60">{label}</span>
    </div>
    <div className="flex items-baseline gap-2">
      <span className="text-3xl lg:text-5xl font-bold text-slate-900 tracking-tighter">{value}</span>
      {unit && <span className="text-[10px] lg:text-xs text-slate-400 font-bold uppercase tracking-widest">{unit}</span>}
    </div>
  </div>
);
